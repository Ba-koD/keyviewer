        ...
        if process_id != 0 {
            ...
            if let Ok(handle) = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, process_id) {
                ...
                if len > 0 {
                    ...
                    eprintln!("Debug: K32GetProcessImageFileNameW returned empty for pid={}", process_id);
                }
            } else {
                #[cfg(debug_assertions)]
            ...
            if process_name.is_empty() {
                if let Ok(handle) = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, false, process_id) {
                    ...
                    if len > 0 {
                        ...
                        #[cfg(debug_assertions)]
                        eprintln!("Debug: GetModuleBaseNameW returned empty for pid={}", process_id);
                    }
                } else {
                    #[cfg(debug_assertions)]
    ...
    unsafe {
        ...
        for i in 0..window_list.len() {
            if let Some(item_ref) = window_list.get(i) {
                ...
                let window_info: CFDictionary<CFString, CFType> = CFDictionary::wrap_under_get_rule(dict_ptr as CFDictionaryRef);
                
                // Check window layer (0 = normal application window)
                let layer = window_info.find(window_layer_key.as_concrete_TypeRef())
                    .and_then(|v| {
                        let num: CFNumber = CFNumber::wrap_under_get_rule(v.as_CFTypeRef() as CFNumberRef);
                        num.to_i64()
                    })
                    .unwrap_or(999);
                ...
                let owner_name = window_info.find(owner_name_key.as_concrete_TypeRef())
                    .and_then(|v| {
                ...
                let window_name = window_info.find(window_name_key.as_concrete_TypeRef())
                    .and_then(|v| {
                        let s: CFString = CFString::wrap_under_get_rule(v.as_CFTypeRef() as CFStringRef);
                        Some(s.to_string())
                    })
                ...
                let window_number = window_info.find(window_number_key.as_concrete_TypeRef())
                    .and_then(|v| {
                        let num: CFNumber = CFNumber::wrap_under_get_rule(v.as_CFTypeRef() as CFNumberRef);
                        num.to_i64()
                    })
                ...
                #[cfg(debug_assertions)]
                eprintln!("[macOS] Window - Layer: {}, Owner: '{}', Title: '{}', Number: {}", 
                         layer, owner_name, window_name, window_number);
                
                // Static warning for empty titles (only show once per run)
                static TITLE_WARNING_SHOWN: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
